...
FROM            (SELECT FAB.ASSET_NUMBER,
 ...
 WHERE 1 = 1
   ...
   AND REP.FISCAL_YEAR = :P_YEAR

     AND REP.FISCAL_YEAR = :P_YEAR
     AND(:P_COMPANY IS NOT NULL REP.COMPANY = :P_COMPANY)
     AND(:P_LIBRO IS NOT NULL REP.BOOK_TYPE_CODE = :P_LIBRO)
     AND (:P_MONEDA IS NOT NULL REP.MONEDA = :P_MONEDA)
     AND(:P_FASE IS IS NOT NULL REP.FASE = :P_FASE)
     AND(:P_CGU IS NULL OR REP.CGU = :P_CGU)


   AND REP.FISCAL_YEAR = :P_YEAR
   AND REP.COMPANY IN (:P_COMPANY,REP.COMPANY)
   AND REP.BOOK_TYPE_CODE IN NVL(:P_LIBRO, REP.BOOK_TYPE_CODE)
   AND REP.MONEDA IN (:P_MONEDA,REP.MONEDA)
   AND REP.FASE IN NVL(:P_FASE,REP.FASE)
   AND REP.CGU IN NVL(:P_CGU,REP.CGU)




   AND REP.FISCAL_YEAR = :P_YEAR
      AND REP.FISCAL_YEAR IN (:P_YEAR, REP.FISCAL_YEAR)
   AND REP.COMPANY IN (:P_COMPANY,REP.COMPANY)
   AND REP.BOOK_TYPE_CODE IN NVL(:P_LIBRO, REP.BOOK_TYPE_CODE)
   AND REP.MONEDA IN (:P_MONEDA,REP.MONEDA)
   AND REP.FASE IN NVL(:P_FASE,REP.FASE)
 ...
 ORDER BY REP.BOOK_TYPE_CODE, REP.ASSET_NUMBER
    ...
    SELECT * 
    FROM AP_INVOICES_ALL AS AIA 
    INNER JOIN AP_INVOICE_PAYMENTS_ALL AIPA ON AIA.INVOICE_ID = AIPA.INVOICE_ID