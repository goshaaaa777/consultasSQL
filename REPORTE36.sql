...
SELECT HEADER_ID,
       ...
       NVL((SELECT SUM(PLA.UNIT_PRICE * PLL.QUANTITY_RECEIVED)
        ...
        NVL( (SELECT SUM(NVL(AIA.invoice_amount,0))
             ...
             WHERE AIA.PO_HEADER_ID IN (SELECT DISTINCT PL.PO_HEADER_ID
                                        FROM PO_LINES_ALL PL
                                        WHERE DECODE(type_lookup_code, 'BLANKET',PL.FROM_HEADER_ID , PL.PO_HEADER_ID) = HEADER_ID--FROM_HEADER
                                       ) 
           ...
           )  / RATE_USD ) AS INVOICED_VALUE_CURRENCY,
    	 ...
    	 NVL( (SELECT SUM(NVL(AIA.invoice_amount,0)) 
            ...
            (SELECT SUM(NVL(AIA.invoice_amount,0)) 
             ...
             AND AIA.INVOICE_CURRENCY_CODE = 'USD' 
            )  *  RATE_USD ) AS INVOICES_VALUE_COP,
           ...
           WHEN 'COP' THEN
                 (AMOUNT - (SELECT SUM(PLA.UNIT_PRICE * PLA.QUANTITY)
                           FROM  PO_LINES_ALL PLA
                           WHERE PLA.FROM_HEADER_ID = HEADER_ID 
                          )) * RATE_USD      --AMOUNT - EXECUTED_WO, 
    	   END , 0.00)  AS BALANCE_TO_DATE_USD, --AMOUNT - EXECUTED_WO * RATE_USD

           NVL(CASE type_lookup_code 
           WHEN 'BLANKET' THEN 
                   CASE WHEN AMOUNT IS NULL OR AMOUNT = 0 THEN NULL
                   WHEN  (SELECT SUM(PLA.UNIT_PRICE * PLA.QUANTITY)
                                             FROM  PO_LINES_ALL PLA
                                             WHERE PLA.FROM_HEADER_ID = HEADER_ID ) IS NULL THEN 0
                   ELSE     
                   -- executed_wo / AMOUNT
                    ...
                    END                                     
            ELSE NULL 
            END , 0.00)AS percentage_comm_wo,
            ...
            END , 0.00) AS PERCENTAGE_AMENDED, --invoices_value / AMOUNT

    	   NVL(CASE WHEN ( case when currency_code = 'USD' 
                	   THEN AMOUNT
                	   ELSE
                	        AMOUNT 
                	   END 
    	    ...
    	    end , 0.00)  AS COND_5K_USD,                	 
		   --CASE WHEN amount * rate_usd > 5000 then 'Y'
    	   case when attribute1 in ('Y','SI') then 'Y' else 'N' end ifrs16,	   
    	   case when attribute1 in ('Y','SI') then 'LEASING' else 'SERVICE' end service_type,
    	   --case when end_date - start_date >= 365  then 'Y' else 'N' end contract_year
    	   case when (decode(type_lookup_code,'BLANKET',trunc(end_date),'STANDARD',
                            			(select min(plla.need_by_date)  
                            			 from PO_LINE_LOCATIONS_ALL plla
                            			 WHERE plla.po_header_id = HEADER_ID)) - 
                      decode(type_lookup_code,'BLANKET',trunc(start_date),'STANDARD',
                            			(select max(pah.action_date) 
                            			 from PO_ACTION_HISTORY pah
                            			 where pah.object_id = HEADER_ID
                            			 and   pah.object_type_code = 'PO'
                            			 and   pah.action_code = 'APPROVE') )) >= 365  
    	   then 'Y' 
    	   else 'N' 
    	   ...
    	   --NULL AS COMMENTS
FROM (
    SELECT ph.po_header_id AS HEADER_ID,
		   (SELECT initcap(pnf1.first_name || ' ' || pnf1.last_name)
            FROM  per_person_names_f pnf1
            WHERE ph.agent_id = pnf1.person_id 
            and pnf1.name_type = 'GLOBAL'
            ) comprador,   
            to_char(trunc(ph.creation_date),'Month','NLS_DATE_LANGUAGE = Spanish') mes_creacion,  
    	   ...
    	   decode(ph.type_lookup_code,'BLANKET','Contrato','STANDARD','Orden') tipo,
    	   ...
    	   (SELECT substr(gllv.ledger_name,1,2) 
            ...
            AND   hov.status = 'A'
            AND   hov.organization_id = ph.prc_bu_id 
           ) AS company_nro,
    	   ...
    	   ROUND(MONTHS_BETWEEN(decode(ph.type_lookup_code,'BLANKET',trunc(ph.end_date),'STANDARD',
                            			(select min(plla.need_by_date) need_by_date
                            			 from PO_LINE_LOCATIONS_ALL plla
                            			 WHERE plla.po_header_id = ph.po_header_id)),  
                                decode(ph.type_lookup_code,'BLANKET',trunc(ph.start_date),'STANDARD',
                            			(select max(pah.action_date) 
                            			 from PO_ACTION_HISTORY pah
            ...
            ROUND(  decode(ph.type_lookup_code,'BLANKET',trunc(ph.end_date),'STANDARD',
                            			(select min(plla.need_by_date) need_by_date
                            			 from PO_LINE_LOCATIONS_ALL plla
                            			 WHERE plla.po_header_id = ph.po_header_id)) - 
                    decode(ph.type_lookup_code,'BLANKET',trunc(ph.start_date),'STANDARD',
    	   ...
    	   decode(ph.type_lookup_code,'BLANKET',trunc(ph.end_date),'STANDARD',
            			(select min(plla.need_by_date) need_by_date
            			 from PO_LINE_LOCATIONS_ALL plla
            			 WHERE plla.po_header_id = ph.po_header_id)
            	) end_date,
           case when decode(ph.type_lookup_code,'BLANKET',trunc(ph.end_date),'STANDARD',
                			(select min(plla.need_by_date) need_by_date
                			 from PO_LINE_LOCATIONS_ALL plla
                			 WHERE plla.po_header_id = ph.po_header_id)
            	            )  > sysdate 
           then    
                FLOOR(MONTHS_BETWEEN( decode(ph.type_lookup_code,'BLANKET',trunc(ph.end_date),'STANDARD',
                                        			(select min(plla.need_by_date) need_by_date
                                        			 from PO_LINE_LOCATIONS_ALL plla
                                        			 WHERE plla.po_header_id = ph.po_header_id)
                                    	            ),(TRUNC(SYSDATE,'MM')-1))) 
    	   ...
    	   then 
    	        FLOOR(decode(ph.type_lookup_code,'BLANKET',trunc(ph.end_date),'STANDARD',
                			(select min(plla.need_by_date) need_by_date
                			 from PO_LINE_LOCATIONS_ALL plla
                			 WHERE plla.po_header_id = ph.po_header_id)
            	            ) - (TRUNC(SYSDATE,'MM')-1) )
    	   ...
    	   --amount * rate_to_cop amount_cop,		
  		   decode(ph.type_lookup_code,'BLANKET',ph.blanket_total_amount,'STANDARD',
            			 (select sum(pda.TAX_EXCLUSIVE_AMOUNT) + sum(pda.NONRECOVERABLE_TAX) 
		   ...
		   NVL((SELECT GDR.CONVERSION_RATE
        	...
        	AND   TO_CHAR(GDR.CONVERSION_DATE,'DD/MM/YYYY') = NVL(TO_CHAR(ph.start_date,'DD/MM/YYYY'), TO_CHAR(PH.CREATION_DATE,'DD/MM/YYYY'))
         ...
         inner join por_requisition_lines_all prla on prla.po_header_id = ph.po_header_id ---
         and prla.po_line_id = pl.po_line_id
         inner join por_requisition_headers_all prh on prh.requisition_header_id = prla.requisition_header_id ---
...
SELECT HEADER_ID,
       ...
       /*(select MAX(PRH.REQUISITION_NUMBER)
       from
  ...
  FROM (SELECT ph.po_header_id AS HEADER_ID,
               ...
               ROUND(MONTHS_BETWEEN(decode(ph.type_lookup_code,
                                           ...
                                           (select promised_date
                                              from (select trunc(plla.promised_date) promised_date
                                                      from po_line_locations_all plla
                                                     where plla.po_header_id = ph.po_header_id
               ...
               decode(ph.type_lookup_code,
                      ...
                      (select max(pah.action_date) action_date
                        ...
                        where pah.object_id = ph.po_header_id
                          and pah.object_type_code = 'PO'
                          and pah.action_code = 'APPROVE')*/
                      trunc(ph.creation_date)) start_date,
               decode(ph.type_lookup_code,
                      'BLANKET',
                      trunc(ph.end_date),
                      'STANDARD',
