SELECT cpd.payment_document_name c_check_stock_name,
  ...
  join iby_ext_bank_accounts ieba on ieba.ext_bank_account_id = ch.external_bank_account_id
  left join ce_payment_documents cpd on cpd.payment_document_id = ch.payment_document_id --EDITED BY PKOLAN   
  join ce_bank_acct_uses_all cbau on ch.ce_bank_acct_use_id = cbau.bank_acct_use_id
  join ce_bank_accounts ba on cbau.bank_account_id = ba.bank_account_id
  join ce_bank_branches_v br on ba.bank_branch_id = br.branch_party_id
  left join poz_supplier_sites_v pssv on ch.vendor_id = pssv.vendor_id and ch.vendor_site_id = pssv.vendor_site_id
  join ap_lookup_codes lk2 on lk2.lookup_code = ch.status_lookup_code and lk2.lookup_type = 'CHECK STATE'
  left join fnd_territories_vl ft on ch.country = ft.territory_code
  left join HZ_PARTY_SITES hps on ch.remit_to_address_id = hps.party_site_id
  left join hZ_LOCATIONS hzl on hps.location_id = hzl.location_id
-------------
  join AP_INVOICE_PAYMENTS_ALL aipa on ch.check_id = aipa.check_id
  join AP_INVOICES_ALL aia on ch.legal_entity_id = aia.legal_entity_id and aipa.invoice_id = aia.invoice_id
